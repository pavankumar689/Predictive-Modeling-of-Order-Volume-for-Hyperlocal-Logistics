...
def main():
    ...
    # Zone ID (simplified for demo)
    zone_options = [f"Zone_{i}" for i in range(1, 21)]
    zone_id = st.sidebar.selectbox("Zone ID", zone_options, help="Select the delivery zone")
    
    # Zone Type
    zone_types = ['Urban', 'Suburban', 'Metropolitan']
    ...
    with col1:
        ...
        with insights_col2:
            st.markdown("""
            **🏙️ Zone Strategy**
            - Commercial zones: lunch rush focused
            - Mixed zones: balanced throughout day  
            - Residential: dinner peak demand
            - Student Areas: late night orders
